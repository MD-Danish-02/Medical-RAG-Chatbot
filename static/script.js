$(document).ready(function() {
            ...
            let isLoggedIn = false; // ← guest/logged-in track karne ke liye

            loadHistoryFromBackend();
            updateBadge();
            loadProfile(); // ← Profile load on startup
            ...
            function loadProfile() {
                $.ajax({
                    ...
                    error: function() {
                        // Guest hai — login button dikhao
                        isLoggedIn = false;
                        $('#login-header-btn').css('display', 'flex');

                        // Profile dropdown me bhi guest message
            ...
            window.toggleProfileMenu = function() {
                // Guest hai to login modal dikhao, dropdown nahi
                if (!isLoggedIn) {
            ...
            function sendMessage() {
                ...
                // Guest hai to login modal dikhao, message send mat karo
                if (!isLoggedIn) {
                    openModal('login-modal');
                    return;
                }
          ...
          <div class="typing-indicator">
              <span></span><span></span><span></span>
                ...
                $.ajax({ data: { msg: raw }, type: 'POST', url: '/get' })
                    ...
                    .fail(function(xhr) {
                        $('#typing-row').remove();
                        // Double safety — agar 401 aaya to bhi modal dikhao
                        if (xhr.status === 401) {
                            openModal('login-modal');
                        } else {
...
// ✅ UPDATED — Backend me save karta hai
...
// ✅ UPDATED — window.open → window.location.href (blank page fix)
