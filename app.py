...
# --- Single-pass retrieval + threshold filter -------------------------
# NOTE: this replaces the old two-step flow (a separate
# similarity_search_with_relevance_scores() call for the threshold check,
# followed by a second, independent retrieval inside RetrievalQA). That
# pattern hit Pinecone twice per user message. Here we retrieve once,
# filter by score, and feed the surviving docs straight into the prompt.
def retrieve_with_threshold(query, k=5, threshold=0.35):
    results = docsearch.similarity_search_with_relevance_scores(query, k=k)
...
def chat():
    ...
    # Step 1: Retrieve + threshold filter (single Pinecone call)
    relevant_docs = retrieve_with_threshold(msg, k=5, threshold=0.5)
    ...
    # Step 2: Build context from the same docs and call the LLM directly.
    # The retrieved text can contain literal { or } characters (citations,
    # ranges, etc.) which crash str.format() if not escaped — that crash
    # was the cause of some questions returning no answer at all. We also
    # wrap the call in try/except so a Groq/API hiccup returns a message
    # instead of silently failing.
    context = "\n\n".join(doc.page_content for doc in relevant_docs)
    safe_context = context.replace("{", "{{").replace("}", "}}")

    try:
        formatted_prompt = PROMPT.format(context=safe_context, question=msg)
        llm_response = llm.invoke(formatted_prompt)
        response = llm_response.content
    except Exception as e:
        print("LLM/prompt error:", e)
        return jsonify({
            "answer": "<p>Something went wrong while generating the answer. Please try again.</p>",
            "sources": []
        }), 200
